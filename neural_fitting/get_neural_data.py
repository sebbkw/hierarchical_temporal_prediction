...
chunk_size = 300
brain_area = 'VISam'
cache_dir = '' # cache dir for the allensdk
    ...
    if False:
        ...
        for movie_key, movie in raw_movie_frames.items():
            ...
            for i in range(0, len(processed_data), chunk_size):
                clip         = processed_data[i:i+chunk_size]
                chunked_clip = []
                
                for j in range(0, clip.shape[0], 5):
                    chunked_clip.append(clip[j:j+5].mean(axis=0))
                chunked_clip = np.array(chunked_clip)
                
                print(clip.shape, chunked_clip.shape)
                
                stimuli_data[f'{movie_key}_{i}'] = chunked_clip

    # Save data
    print('\n\nSaving data with', len(neural_data.keys()), 'cells')

...
def get_quadrant_mask (q):
    h = np.ones((102, 102))
    h_0 = np.tril(h)
    h_1 = np.rot90(np.tril(np.rot90(h)), k=-1)
    h_2 = np.triu(h)
    h_3 = np.rot90(np.triu(np.rot90(h)), k=-1)
...
def get_z_score_proportion (model_a, model_b, target_a, target_b):
    p_model  = model_a/model_b
    p_target = target_a/target_b
    
    p_pooled = (model_a+target_a)/(model_b+target_b)
    
    se = (p_pooled*(1-p_pooled)*(1/model_b+1/target_b))**0.5
    
    z     = (p_target-p_model)/se

    return z

def get_z_score_pearson_r (model, target):
    r, _ = stats.pearsonr(model, target)
...
def save_connectivity_data (model_name, measure_name, weight, z):
    ...
    if measure_name in data[model_name]:
        raise Exception("This will over-write existing data!")
    else:
        data[model_name][measure_name] = (z, weight)
        
    np.save('connectivity_data.npy', data)
    
# Stouffer's Z-score method
def get_connectivity_data_z ():
    average_scores = {}
    ...
    for model_name, model_data in data.items():
        ...
        for score, weight in model_data.values():
        ...
        weighted_average = np.sum(score_arr*weight_arr)
        se               = np.sqrt(np.sum(weight_arr**2))
        
        average_scores[model_name] = 2 * (1 - stats.norm.cdf(weighted_average/se))
        
    return average_scores


def save_connectivity_data_z (model_name, measure_name, weight, z):
    # Load existing dataset
    try:
        data = np.load('connectivity_data.npy', allow_pickle=True).item()
    except:
...
def save_connectivity_data (model_name, measure_name, model, target):
    # Load existing dataset
    try:
        data = np.load('connectivity_data_2.npy', allow_pickle=True).item()
    except:
        data = {}
    ...
    np.save('connectivity_data_2.npy', data)
...
def get_connectivity_data ():
    ...
    for model_name, model_data in data.items():
        ...
        average_scores[model_name] = np.mean(cc_arr), np.std(cc_arr)/(len(cc_arr)**0.5)
